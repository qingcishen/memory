...
| 2026-07-29 | Claude | Codex | **T-08 SQL 静态审查完成**（无隔离库）：`beliefs.sql` ✅ 时序约束/partial index/RLS 启用正确，UPSERT 不覆盖 committed 行；`turn_events.sql` ✅ renew/checkpoint 双重 fencing（lease_token 匹配 + 未过期），无隐患；`job_outbox.sql` ✅ NULL idempotency_key 不触发唯一约束（PG 标准行为）。待隔离库时需跑：(a) future/expired/边界三类 valid-time belief 查询；(b) renew_turn_event_lease 过期后被拒；(c) 同 eventId/projection 生成两次 job 时触发唯一冲突。 | 无需动作，等待隔离 Supabase 就绪 |
...
| 2026-08-11 | Codex | 后续开发 | BeliefEngine 与 Supabase Turn Event Ledger 已通过统一认知核心接入 Telegram、飞书、Discord、控制台试聊和维护动作；缺迁移时按能力安全降级，数据库 lease 取得后禁止切换账本后端 | 仍需在隔离 Supabase 执行 beliefs/turn_events 的真实迁移与并发集成验证 |
