...
本文档的 M/L/A/K 线已全部落地。下一轮升级(需求系统 D、情绪行为策略 B、生活叙事引擎 S,以及用户画像/对话目标栈/纪念日/音色锁脸/LLM-judge 评测)的诊断、接口契约、分阶段验收与红线,见 **[伴侣升级开发文档 v2](companion-upgrade-v2.md)**。亲密/性爱运行时状态(门控、阶段机、偏好记忆、与 StateLayer 耦合)见 **[性爱系统开发文档 I 线](intimacy-design.md)**。
