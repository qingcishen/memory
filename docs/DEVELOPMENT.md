...
2. **状态历史表**:待补。`affective_state` 目前只有当前快照,`narrative.js` 无法看到关系如何从生疏到亲密、从冷战到和好。下一步应新增 `affective_state_events` 或 `affective_state_history`,在 `updateFromTurn/decayToBaseline` 后记录状态 delta 与原因,并让 `composeNarrativeInput` 读取轨迹。
