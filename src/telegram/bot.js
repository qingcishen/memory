 ...
 *  半角() 一律删。"……"/"......" 做分隔符是文学写法, 删。
 *  第三人称动作句（"她/清词+动词" 独立成句）也删——这是对话后跟着的旁白尾巴。 */
function stripNarration(text = '') {
  return text
    .replace(/（[^）]{8,}）/g, '')               // 长全角括号 = 旁白, 删
    .replace(/\([^)]*\)/g, '')                    // 半角括号, 删
    .replace(/\*[^*]+\*/g, '')                    // *markdown动作*, 删
    .replace(/[：:]\s*[.…]{2,}/g, '')             // 冒号+省略号, 删
    .replace(/[.…]{4,}/g, '')                     // 4个以上省略号分隔符, 删
    // 第三人称旁白句: 以 她/清词 开头紧跟动词的独立句子 (句末有。！？或行尾), 删
    .replace(/(?:^|\n)[　\s]*(她|清词)[^，。！？\n]{0,3}(?:脱|爬|慢慢|轻轻|低头|抬头|偏头|伸手|张开|闭上|转过|侧身|靠近|凑近|移动|坐上|趴|躺|站)[^\n]*[。！？]?/g, '')
    .replace(/\n{2,}/g, '\n')                     // 段落空行 → 单换行
    .replace(/^[.…·。\s]+/, '')                   // 开头停顿符
    .trim();
}
