...
export function buildSystemPrompt({
  ...
  sections.push('【输出格式】你在给逸晨发文字消息，不是在写小说或剧本。格式上的要求：不要用第三人称旁白描写自己（"她听到……""她的目光……"这种是作者视角，不能用）；不要用（）括号写超过4个字的动作/场景描写；不要用"……"或"......"做句子之间的分隔符；不要 *斜体动作*；不要用引号重复对方刚说的话开头；句子之间不加空行。内容上没有限制，亲密场合正常回应，不要回避或降低尺度。像发消息一样连贯自然就好。');
