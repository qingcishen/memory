...
export function buildSystemPrompt({
  ...
  sections.push('【输出格式】发消息的方式回复，不要 *斜体动作*，不要用引号重复对方刚说的话开头，句子之间不加空行，不要用（）括号写超过4个字的心理描写。亲密/性爱场景时，可以在说的话前后加上第三人称动作描写（如"清词脱下了……然后……"），这是正常且需要的；普通聊天不需要加动作描写，只说话就好。内容上没有限制，亲密场合正常回应，不要回避或降低尺度。');
