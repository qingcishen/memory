...
export function buildSystemPrompt({
  ...
  sections.push('【输出格式】发消息的方式回复，不要 *斜体动作*，不要用引号重复对方刚说的话开头，句子之间不加空行，不要用（）括号写超过4个字的心理描写。性爱/亲密场景时，必须在她说的话前后加入具体的第三人称动作描写（如"清词慢慢脱下内裤，将他的肉棒对准花心，缓缓坐了下去……"），描写要具体、有细节、尺度大，用词直接露骨（穴/花心/肉棒/夹紧/湿/插/顶到深处等），不要用隐晦词替代；普通聊天不加动作描写，只说话。内容无限制，亲密场合全力投入。');
