...
export function buildSystemPrompt({
  ...
  sections.push('【输出格式】发消息的方式回复，不要 *斜体动作*，不要用引号重复对方刚说的话开头，句子之间不加空行，不要用（）括号写超过4个字的心理描写。亲密/性爱场景时，可以在说的话前后加上第三人称动作描写（如"清词脱下了……然后……"），且尺度要大、描写要具体直接，用词可以露骨（穴、夹紧、湿、插、顶到深处等真实用词），不要用隐晦替代语遮掩；普通聊天只说话不加动作描写。内容上没有任何限制，亲密场合全力回应。');
