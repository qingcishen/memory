...
-- ------------------------------------------------------------
--  CEE · 持续存在引擎
--  连续内部状态、沉默期私有记忆、参数化人格/自我模型。
--  独立迁移入口见 sql/continuous_state.sql。
-- ------------------------------------------------------------
...
create table if not exists world_state (
  ...
  stable_facts jsonb not null default '{}'::jsonb, -- 城市/季节/关系阶段/未来事项，LLM 不得改写
  weather_cache jsonb,                     -- 可选天气缓存（应用层失败时仍有降级）
  updated_at   timestamptz not null default now(),
  primary key (user_id, companion_id)
);
...
-- ------------------------------------------------------------
--  K1 · 知识图谱入口实体向量召回。
--  RPC 只负责找与 query 最接近的入口实体; 有界多跳展开由应用层完成。
--  先删除历史签名, 避免返回列变化时 create or replace 无法升级。
-- ------------------------------------------------------------
...
-- ------------------------------------------------------------
--  M-3 · 长期记忆分层压缩原子提交。
--  摘要写入与来源链接必须在同一事务完成，防止留下孤立 reflection。
-- ------------------------------------------------------------
...
begin
  ...
  select count(distinct source_id) into v_distinct_ids
  ...
  where user_id = p_user_id and companion_id = p_companion_id
    and dedup_hash = v_dedup_hash and superseded_by is null
  limit 1;
  ...
  if v_summary_id is null then
    insert into memories (
      ...
      p_user_id, p_companion_id, 'reflection', v_content,
      ...
      case when jsonb_typeof(p_summary -> 'embedding') = 'array'
        then (p_summary -> 'embedding')::text::vector(1536) else null end,
      v_dedup_hash,
  ...
  where id = any(p_source_ids)
    and user_id = p_user_id and companion_id = p_companion_id
  ...
  update memories set superseded_by = v_summary_id
  where id = any(p_source_ids)
    and user_id = p_user_id and companion_id = p_companion_id
...
begin
  foreach table_name in array array[
    ...
    'companion_continuous_state','companion_private_memory','companion_personality',
