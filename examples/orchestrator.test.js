...
console.log('buildSystemPrompt (纯拼接, 跳过空段落; 结尾恒定追加事实/格式规则)');
{
  ...
  const withIdentity = buildSystemPrompt({
    personaPrompt: 'A',
    identityConstraintsPrompt: '【身份设定·硬约束】测试约束',
    narrationPrompt: '【旁白提示】测试指令',
  });
  ok('身份硬约束段落存在', withIdentity.includes('【身份设定·硬约束】测试约束'));
  ok(
    '身份硬约束排在恒定规则之后、旁白指令之前',
    withIdentity.indexOf('【输出格式】') < withIdentity.indexOf('【身份设定·硬约束】') &&
      withIdentity.indexOf('【身份设定·硬约束】') < withIdentity.indexOf('【旁白提示】')
  );
}
...
console.log('Orchestrator.reply 身份硬约束 (CompanionConfig.identityConstraints -> 独立高显著度段落)');
{
  const deps = makeMocks();
  const config = normalizeCompanionConfig({ name: '可可', identityConstraints: ['他是在读大二学生, 不是上班族, 没有工作/公司/加班'] });
  const orch = new Orchestrator({ userId: 'u_identity', deps, config, options: { useMonologue: false } });

  await orch.reply('今天好累');
  const { messages } = deps.llm.generateCalls[0];
  ok(
    'system 段含身份硬约束',
    messages[0].content.includes('【身份设定·硬约束】') && messages[0].content.includes('不是上班族, 没有工作/公司/加班')
  );

  const noConstraintDeps = makeMocks();
  const noConstraintOrch = new Orchestrator({ userId: 'u_no_identity', deps: noConstraintDeps, options: { useMonologue: false } });
  await noConstraintOrch.reply('你好');
  ok(
    '没有 config 时不出现身份硬约束段落',
    !noConstraintDeps.llm.generateCalls[0].messages[0].content.includes('【身份设定·硬约束】')
  );
}
