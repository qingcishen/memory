...
console.log('RelationshipAdapter.bump / StateLayerAdapter.evolve 委托统一状态层');
{
  ...
  const stateLayer = new StateLayerAdapter('u_noop', {
    async snapshot() {
      return null;
    },
    async evolve(turns) {
  ...
  ok('stateLayer.evolve 委托给 StateLayer', (await stateLayer.evolve(turns)) === undefined && delegatedTurns === turns);
